...
// DPF generation
// -------------------------------------------------------------
DPF_key generateDPF(uint32_t index, long long value, ofstream &dpf0, ofstream &dpf1, ofstream &summary)
{
...
// DPF Evaluation
// -------------------------------------------------------------
vector<uint32_t> evalDPF(const node &seed, const vector<correction_word> &data, uint32_t size, ofstream &summary)
{
...
// -------------------------------------------------------------
bool EvalFull(const DPF_key &key, uint32_t target_index, long long target_value, ofstream &summary)
{
    summary << "\n[LOG] Running EvalFull() for target_index=" << target_index
         << ", target_value=" << target_value << "\n";

    vector<uint32_t> arr = evalDPF(key.seed, key.data, DPF_size, summary);
    vector<uint32_t> arr1 = evalDPF(key.seed1, key.data, DPF_size, summary);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < num_DPFs; i++)
    {
        ...
        dpf0 << "DPF " << i + 1 << ":\n";
        dpf1 << "DPF " << i + 1 << ":\n";
        DPF_key key = generateDPF(index, value, dpf0, dpf1, summary);
        dpf0 << "----------------------------------------------\n\n";
        dpf1 << "----------------------------------------------\n\n";
        if(EvalFull(key, index, value, summary))
        {
